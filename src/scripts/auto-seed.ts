...
export async function runSeed() {
  ...
  try {
    await client.execute("ALTER TABLE api_keys ADD COLUMN raw_key TEXT")
  } catch {
  ...
  await client.execute({
    sql: `INSERT INTO api_keys (id, account_id, key_prefix, key_hash, raw_key, label, is_active, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [keyId, accountId, rawKey.slice(0, 10), keyHash, rawKey, "Demo Key", 1, now],
  })
